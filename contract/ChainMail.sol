...
contract ChainMail {
    ...
    function sendMail(address reciever_,string memory subject,string memory body,uint256 timeStamp,string memory hash,string memory filename) public{
        require(msg.sender.balance > 0 , "You may not have enough funds.");
        inbox[reciever_].push(Mail(msg.sender,reciever_,subject,body,timeStamp,hash,filename));
        sentMails[msg.sender].push(Mail(msg.sender,reciever_,subject,body,timeStamp,hash,filename));
